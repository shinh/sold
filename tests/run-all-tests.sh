...
do
    ...
    if [ "$?" -ne 0 ];then
        if [ -z "${unexpected_failed_tests}" ];then
            ...
            unexpected_failed_tests=${dir},${unexpected_failed_tests}
        fi
    fi
    popd
done

for dir in tls-lib-gcc-aarch64 setjmp-gcc-aarch64 stb_gnu_unique_tls-aarch64 exception-g++-aarch64 tls-multiple-module-g++-aarch64 static-in-class-g++-aarch64 static-in-function-g++-aarch64 tls-dlopen-gcc-aarch64 dynamic_cast-g++-aarch64 typeid-g++-aarch64 inheritance-g++-aarch64 call_once-g++-aarch64 tls-thread-g++-aarch64 tls-multiple-lib-gcc-aarch64 tls-lib-gcc-without-base-aarch64 just-return-gcc-aarch64 
do
    pushd `pwd`
    cd $dir
    echo =========== $dir ===========
    ...
    if [ "$?" -eq 0 ];then
        if [ -z "${unexpected_succeeded_tests}" ];then
            ...
            unexpected_succeeded_tests=${dir},${unexpected_succeeded_tests}
        fi
    fi
    popd
done
