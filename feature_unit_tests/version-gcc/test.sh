...
LD_LIBRARY_PATH=. $(git rev-parse --show-toplevel)/build/sold -e libmax.so -o vertest1.out vertest1 --section-headers --check-output 
LD_LIBRARY_PATH=. ./vertest1.out
LD_LIBRARY_PATH=. $(git rev-parse --show-toplevel)/build/sold -e libmax.so -o vertest2.out vertest2  --section-headers --check-output
